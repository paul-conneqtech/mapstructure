...
func ExampleDecodeWithEmbeddedSliceSubset() {
	var document string = `{
	  "cobrandId": 10010352,
	  "channelId": -1,
	  "locale": "en_US",
	...
	type People struct {
		Age         int      `jpath:"age.birth"` // jpath is relative to the array
		AnimalRaces []string `jpath:"age.animals.race"`
	}
	...
	fmt.Printf("%#v", items)
	// Output:
	// mapstructure.Items{Categories:[]string{"rabbit", "bunny", "frog"}, Peoples:[]mapstructure.People{mapstructure.People{Age:10, AnimalRaces:[]string{"dog", "cat"}}, mapstructure.People{Age:11, AnimalRaces:[]string(nil)}}}
}
